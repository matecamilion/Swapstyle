...
export function MetricasClient({ ventas, ventaProductos, productos, gastos }: Props) {
  // ── KPIs globales ──────────────────────────────────────────────────────────
  ...
  const lineData = Object.values(porDia).sort((a, b) => a.fecha.localeCompare(b.fecha)).map((d) => ({
    ...d,
  ...
  // ── Por proveedor ──────────────────────────────────────────────────────────
  const porProveedor = ventaProductos.reduce<Record<string, number>>((acc, vp) => {
    const nombre = vp.productos?.proveedores?.nombre ?? 'Sin proveedor'
    acc[nombre] = (acc[nombre] ?? 0) + vp.precio_venta_momento
    return acc
  }, {})
  const provData = Object.entries(porProveedor)
    .map(([name, total]) => ({ name, total }))
    .sort((a, b) => b.total - a.total)
    .slice(0, 8)
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Línea de ventas últimos 30 días */}
      <section>
      ...
      {/* Por proveedor */}
      <section>
        <SectionTitle>Top proveedores por ventas</SectionTitle>
        {provData.length === 0 ? (
          <div className="rounded-xl border border-[var(--border-subtle)] py-12 text-center text-[var(--text-muted)]">Sin datos</div>
        ) : (
          <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-6">
            <ResponsiveContainer width="100%" height={280}>
              <BarChart data={provData} layout="vertical">
                <CartesianGrid strokeDasharray="3 3" stroke="#222" horizontal={false} />
                <XAxis type="number" tickFormatter={fmt$K} tick={{ fill: '#525252', fontSize: 11 }} />
                <YAxis type="category" dataKey="name" width={120} tick={{ fill: '#A3A3A3', fontSize: 11 }} />
                <Tooltip {...tooltipStyle} formatter={(v) => [formatCurrency(Number(v)), 'Vendido']} />
